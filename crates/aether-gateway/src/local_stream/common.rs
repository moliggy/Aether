...
pub(super) fn map_claude_stop_reason(
...
pub(super) fn encode_done_sse() -> Vec<u8> {
    b"data: [DONE]\n\n".to_vec()
}

pub(super) fn encode_json_sse(event: Option<&str>, value: &Value) -> Result<Vec<u8>, GatewayError> {
    let mut out = Vec::new();
