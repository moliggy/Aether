...
#[tokio::test]
async fn gateway_executes_claude_chat_stream_via_executor_decision() {
        ...
        .route(
            ...
            any(|_request: Request| async move {
                Json(json!({
                    ...
                    "auth_context": {
                        "user_id": "user-claude-stream-decision-123",
                        "api_key_id": "key-claude-stream-decision-123",
                        "access_allowed": true
                    },
                ...
                async move {
                    *decision_hits_inner.lock().expect("mutex should lock") += 1;
                    Json(json!({
                        ...
                        "request_id": "req-claude-chat-stream-decision-123",
                        "candidate_id": "cand-claude-chat-stream-decision-123",
                        "provider_name": "claude",
                        "provider_id": "provider-claude-chat-stream-decision-123",
                        "endpoint_id": "endpoint-claude-chat-stream-decision-123",
                        "key_id": "key-claude-chat-stream-decision-123",
    ...
    let executor = Router::new().route(
        ...
        any(|request: Request| async move {
            ...
            assert_eq!(
                ...
                Some("https://api.anthropic.example/v1/messages")
            );
            assert_eq!(
                payload
                    .get("headers")
    ...
    let response = reqwest::Client::new()
        ...
        .header(TRACE_ID_HEADER, "trace-claude-chat-stream-decision-123")
...
#[tokio::test]
async fn gateway_executes_claude_cli_stream_via_executor_decision() {
    let decision_hits = Arc::new(Mutex::new(0usize));
    let decision_hits_clone = Arc::clone(&decision_hits);
    let plan_hits = Arc::new(Mutex::new(0usize));
    let plan_hits_clone = Arc::clone(&plan_hits);
        ...
        .route(
            ...
            any(|_request: Request| async move {
                Json(json!({
                    ...
                    "executor_candidate": true,
                    "auth_context": {
                        "user_id": "user-claude-cli-stream-decision-123",
                        "api_key_id": "key-claude-cli-stream-decision-123",
                        "access_allowed": true
                    },
                    "public_path": "/v1/messages"
                }))
                ...
                async move {
                    *decision_hits_inner.lock().expect("mutex should lock") += 1;
                    Json(json!({
                        ...
                        "request_id": "req-claude-cli-stream-decision-123",
                        "candidate_id": "cand-claude-cli-stream-decision-123",
                        "provider_name": "claude",
                        "provider_id": "provider-claude-cli-stream-decision-123",
                        "endpoint_id": "endpoint-claude-cli-stream-decision-123",
                        "key_id": "key-claude-cli-stream-decision-123",
                        "upstream_base_url": "https://api.anthropic.example",
                        "upstream_url": "https://api.anthropic.example/v1/messages",
    ...
    let response = reqwest::Client::new()
        .post(format!("{gateway_url}/v1/messages"))
        .header(http::header::CONTENT_TYPE, "application/json")
        .header(TRACE_ID_HEADER, "trace-claude-cli-stream-decision-123")
