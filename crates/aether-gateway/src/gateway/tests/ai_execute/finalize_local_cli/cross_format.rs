...
async fn gateway_executes_openai_cli_cross_format_upstream_stream_via_local_finalize_response() {
    ...
    assert_eq!(seen_report_request.upstream_id, "upstream-cli-stream-123");
    assert_eq!(seen_report_request.client_id, "upstream-cli-stream-123");
...
async fn gateway_executes_openai_cli_cross_format_function_call_upstream_stream_via_local_finalize_response(
    ...
    let seen_report_request = seen_report
        ...
        .expect("report-sync should be captured");
    assert_eq!(seen_report_request.report_kind, "openai_cli_sync_success");
    assert_eq!(seen_report_request.client_id, "upstream-cli-tool-123");
    assert_eq!(*execute_hits.lock().expect("mutex should lock"), 0);
