...
#[tokio::test]
async fn gateway_executes_openai_chat_claude_cross_format_tool_use_stream_via_local_stream_rewrite()
{
        ...
        .route(
            ...
            any(|_request: Request| async move {
                Json(json!({
                    ...
                    "auth_context": {
                        "user_id": "user-openai-chat-stream-claude-tool-xfmt-direct-123",
                        "api_key_id": "key-openai-chat-stream-claude-tool-xfmt-direct-123",
                        "access_allowed": true
                    },
        ...
        .route(
            ...
            any(|_request: Request| async move {
                Json(json!({
                    ...
                    "request_id": "req-openai-chat-stream-claude-tool-xfmt-direct-123",
                    "candidate_id": "cand-openai-chat-stream-claude-tool-xfmt-direct-123",
                    "provider_name": "claude",
                    "provider_id": "provider-openai-chat-stream-claude-tool-xfmt-direct-123",
                    "endpoint_id": "endpoint-openai-chat-stream-claude-tool-xfmt-direct-123",
                    "key_id": "key-openai-chat-stream-claude-tool-xfmt-direct-123",
    ...
    let executor = Router::new().route(
        ...
        any(|_request: Request| async move {
            let claude_stream = concat!(
                "event: message_start\n",
                "data: {\"type\":\"message_start\",\"message\":{\"id\":\"msg_tool_claude_chat_stream_123\",\"type\":\"message\",\"role\":\"assistant\",\"model\":\"claude-sonnet-4-5-upstream\",\"content\":[],\"stop_reason\":null,\"stop_sequence\":null}}\n\n",
    ...
    let response = reqwest::Client::new()
        ...
        .header(
            TRACE_ID_HEADER,
            "trace-openai-chat-stream-claude-tool-xfmt-direct-123",
        )
        .body("{\"model\":\"gpt-5\",\"messages\":[{\"role\":\"user\",\"content\":\"weather\"}],\"stream\":true}")
        .send()
        .await
    ...
    assert!(response_text.contains("\"id\":\"tool_123\""));
    assert!(response_text.contains("\"name\":\"get_weather\""));
    assert!(response_text.contains("\\\"location\\\":\\\"Tokyo\\\""));
